...
N_PHASE1 = 10000    # epochs conjuntas (red + física)
N_PHASE2 = 5000     # epochs solo física (x_leak)

LR_NET   = 1e-3
LR_XLEAK = 0.5     # Used only in Phase 2
...
def loss_pde(model, x_col, t_col, P_scale, sig=SIGMA_SRC):
    """
    ...
    S   = source_torch(xc, tc, model.x_leak, sig=sig)
...
def train(sim):
    ...
    # ═══════════════════════════════════════════════════
    # FASE 1: Annealing (Sigma ancho)
    #         Red y x_leak se entrenan juntos con una fuente difusa
    #         para evitar vanishing gradients.
    # ═══════════════════════════════════════════════════
    print(f"\n{'─'*60}")
    print(f"FASE 1 ({N_PHASE1} epochs) — Annealing con sigma=200.0")
    print(f"{'─'*60}")
    ...
    for epoch in range(1, N_PHASE1 + 1):
        ...
        # Anneal sigma from 300 to 80 over Phase 1
        sig_anneal = 300.0 - 220.0 * (epoch / N_PHASE1)
        Lp = loss_pde(model, x_col, t_col, P_scale, sig=sig_anneal)
        
        loss_total = LAM_DAT*Ld + LAM_BC*Lb + LAM_IC*Li + Lp
        loss_total.backward()
        ...
        if epoch % 1000 == 0:
            ...
            print(f"  ep {epoch:5d} | L={loss_total.item():.3e} | "
                  ...
                  f"x_leak={xl:.1f}m | sig={sig_anneal:.1f}")
            history.append({"phase":1, "epoch":epoch,
                             "x_leak":xl, "L_dat":Ld.item(),
                             "L_pde":0.0, "L_bc":Lb.item()})

    # Eliminamos el escaneo diagnóstico, ya que x_leak convergerá de forma natural

    # ═══════════════════════════════════════════════════
    # FASE 2: Congelamos la red y optimizamos SOLO x_leak
    # ═══════════════════════════════════════════════════
    print(f"\n{'─'*60}")
    print(f"FASE 2 ({N_PHASE2} epochs) — Optimizando x_leak con red fija")
    print(f"{'─'*60}")

    for p in model.network_params():
        p.requires_grad = False
    model.x_leak_raw.requires_grad = True
    ...
    for epoch in range(1, N_PHASE2 + 1):
        opt2.zero_grad()
        Lp  = loss_pde(model, x_col, t_col, P_scale, sig=80.0)
        Lp.backward()
        opt2.step(); sch2.step()
