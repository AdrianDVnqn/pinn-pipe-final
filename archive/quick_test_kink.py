...
from pinn_model_kink import train_pinn

TEST_SCENARIOS = [
    {
        "scenario_id": 1,
        "descripcion": "Fuga cerca de entrada, caudal pequeño",
        "x_leak_true": 2000,
        "q_leak_true": 0.005,
        "dificultad": "MÁXIMA",
    },
    {
        "scenario_id": 4,
        "descripcion": "Fuga cerca de salida, caudal pequeño",
        "x_leak_true": 8000,
        "q_leak_true": 0.005,
        "dificultad": "ALTA",
    },
    {
        "scenario_id": 9,
        "descripcion": "Fuga cerca de entrada, caudal grande",
        "x_leak_true": 2000,
        "q_leak_true": 0.030,
        "dificultad": "MEDIA",
    },
    {
        "scenario_id": 12,
        "descripcion": "Fuga cerca de salida, caudal grande",
        "x_leak_true": 8000,
        "q_leak_true": 0.030,
        "dificultad": "MEDIA",
    },
...
def run_single_test(scenario):
    ...
    history = train_result["history"].to_dict('records') # Convert pandas DataFrame back to list of dicts
...
def plot_quick_test(results, save_dir="figs"):
    ...
    for i, r in enumerate(results):
        ...
        l_total = [h["loss_total"] for h in hist]
        l_p = [h["L_P"] for h in hist]
        l_q = [h["L_Q"] for h in hist]
        l_pde = [h["L_fisica"] for h in hist]
...
def save_results(results, reco, exitos):
    ...
    out = {
        ...
        "global_result": f"{exitos}/4 exitos",
        "recommendation": reco
    }
...
if __name__ == '__main__':
    ...
    print("  ═══════════════════════════════════════════════")
    print("Corriendo quick test en 4 escenarios extremos...")
    print("Tiempo estimado: ~30-40 minutos\n")

    results = []
    for i, scenario in enumerate(TEST_SCENARIOS):
        print(f"[{i+1}/4] Escenario {scenario['scenario_id']} | x_leak={scenario['x_leak_true']}m | q={scenario['q_leak_true']} | dificultad={scenario['dificultad']}")
        res = run_single_test(scenario)
        results.append(res)
