...
class LeakPINN(nn.Module):
    def __init__(self, hidden_layers: int = 5, hidden_size: int = 64):
        ...
        # Trainable magnitude for the pressure kink
    ...
    def network_params(self):
        ...
        exclude = {id(self.x_leak_raw), id(self.q_leak_raw), id(self.beta)}
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        P_norm = out[:, 0]
        Q_norm = out[:, 1]
        P_smooth = P_norm.reshape(orig_shape) * self.P_in
        Q_smooth = Q_norm.reshape(orig_shape) * self.Q_out
        
        # Hard-coded physics prior: Inject the spatial kink in the pressure
        P_final = P_smooth + self.beta * torch.abs(x - self.x_leak)
        
        return P_final, Q_smooth
...
def leak_source(x: torch.Tensor, t: torch.Tensor, x_leak: torch.Tensor, q_leak: torch.Tensor, sigma: float):
    tau = 2.0
...
def compute_pde_residuals(model: LeakPINN, x_col: torch.Tensor, t_col: torch.Tensor, sigma: float):
    device = x_col.device
    ...
    # x_leak MUST NOT be detached here, otherwise its gradient is zero since it's no longer in the network inputs.
    S = leak_source(x_col, t_col, model.x_leak, model.q_leak.detach(), sigma)
...
def compute_loss(model: LeakPINN, data_dict: Dict, x_col: torch.Tensor, t_col: torch.Tensor, lambdas: Dict, sigma_current: float):
    device = x_col.device
    mse = nn.MSELoss()
    ...
    # PDE residual loss
    r_cont, r_mom = compute_pde_residuals(model, x_col, t_col, sigma_current)
    ...
    # ── L_masa: Global mass balance to estimate q_leak ──
    # The mean difference between inlet and outlet flows in the last 20% of the simulation
...
def train_pinn(scenario_id=7,
    ...
    optimizer = torch.optim.Adam([
        ...
        {'params': [model.x_leak_raw], 'lr': 5e-3, 'name': 'x_leak'},
        {'params': [model.q_leak_raw], 'lr': 5e-3},
        {'params': [model.beta], 'lr': 5e-3},
    ])
    ...
    for epoch in range(1, actual_adam_epochs + 1):
        ...
        sigma_current = compute_sigma(epoch, actual_adam_epochs, sigma_start, sigma_end)
        ...
        optimizer.zero_grad()
        loss_total, comps, comps_tensors = compute_loss(model, {**device_data}, x_col, t_col, lambdas, sigma_current)
        ...
        if epoch <= 5:
            ...
            if grad_data_norm > 10.0 * grad_pde_norm and epoch == 1:
                log.info(f"Epoch {epoch}: Grad L_P ({grad_data_norm:.3e}) >> Grad L_pde ({grad_pde_norm:.3e})")
                log.info("Esto causa que L_pde no pueda competir inicialmente. Usando annealing dinámico hasta 1000.0")
        ...
        if epoch == epoch_fase3:
            for pg in optimizer.param_groups:
                # We will check if it's the x_leak_raw parameter group.
                # Since we didn't add a 'name' explicitly to the dict, we use index 1.
        ...
        if verbose and (epoch == 1 or epoch % progress_every == 0 or epoch == actual_adam_epochs):
            ...
            log.info(
                f"Epoch {epoch:5d} | "
                f"Loss: {loss_total:.3e} | "
                f"L_P: {comps['L_P']:.3e} | "
                f"L_Q: {comps['L_Q']:.3e} | "
                f"L_fisica: {comps['L_fisica']:.3e} | "
                f"L_masa: {comps['L_masa']:.3e} | "
                f"x_leak: {model.x_leak.item():.0f}m | "
                f"q_leak: {model.q_leak.item():.5f} | "
                f"σ: {sigma_current:.0f}m | "
    ...
    if use_lbfgs and actual_lbfgs_epochs > 0:
        ...
        for step in range(1, actual_lbfgs_epochs + 1):
            ...
            def closure():
                ...
                loss_total, comps, comps_tensors = compute_loss(model, device_data, x_col_fixed, t_col_fixed, lambdas, sigma_end)
                loss_total.backward()
                comps_lbfgs = comps
